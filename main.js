...
function preload() {
    ...
    game.load.image("p2",'assets/Mini Boss mage (1).png');
    game.load.image('box', 'assets/box.png');
    game.load.image('arrow',"assets/Arrow.png");
    game.load.spritesheet('boss','assets/boss (1).png',250,250);
...
function create() {
    ...
    //Ammo Pickup
    ammoPickUp = game.add.group();
    ammoPickUp.enableBody = true;
    ammunition = ammoPickUp.create(425, game.world.height - 130, 'ammunition');
    //P2 Attack
    ...
    //Create Grappling hook for player 1
    gHook = game.add.sprite(-1000, -1000, 'arrow');
    game.physics.enable(gHook);
    ...
    nineKey = game.input.keyboard.addKey(Phaser.Keyboard.NUMPAD_9);
...
function update() {
    ...
    //Grapple Perform
    if (game.physics.arcade.overlap(gHook, boss)) {
        player.x = boss.x;
    }
   ...
   //Player 1 Movement
    ...
    if (nineKey.isDown) {
        gHook = game.add.sprite(player.x,player.y, 'arrow');
        game.physics.enable(gHook);
        for(i=0; i<= 50; i++) {
            gHook = game.add.sprite(gHook.x, gHook.y, 'arrow');
            gHook.x += .5;
            if (i == 10) {
                for (r=0; r<= 50; r++) {
                    player.x += .5;
